...
class MainWindow(QMainWindow):
    ...
    def enviar_pergunta(self):
        """
        Envia a pergunta para o processamento e gera uma resposta usando o Catelina Lacet GPT.
        """
        prompt = self.input_field.text()
        try:
            # Primeiro envia o prompt para o processamento de texto
            analysis = self.cateline_lacet_gpt.text_processor.process_prompt(prompt)

            if self.cateline_lacet_gpt.text_processor.is_greeting(prompt):
                resposta = self.cateline_lacet_gpt.greeting_service.get_greeting_response(analysis['sentiment'])
            else:
                # Caso contrário, continue com o processamento da pergunta
        ...
        if file_path:
            try:
                ...
                with open(file_path, 'rb') as f:
                    file_data = f.read()
                self.file_data = file_data  # Salvar os dados do arquivo para as análises posteriores
                self.analyze_cost_button.setEnabled(True)
                self.analyze_financial_button.setEnabled(True)
                self.result_display.append(f"<p style='color: cyan;'>Arquivo carregado com sucesso.</p>")
            except Exception as e:
                self.result_display.append(f"<p style='color: red;'>Erro ao processar o arquivo: {str(e)}</p>")

    def analyze_cost(self):
        """Realiza a análise de custos usando os dados financeiros carregados."""
        if self.file_data:
            try:
                resposta = self.cateline_lacet_gpt.get_financial_analysis_response(self.file_data, horas_trabalhadas=40)
                self.result_display.append(f"<p style='color: yellow;'>Análise de Custos:\n{resposta}</p>")
            except Exception as e:
                self.result_display.append(f"<p style='color: red;'>Erro ao realizar a análise de custos: {str(e)}</p>")

    def analyze_financial(self):
        """Realiza a análise financeira usando os dados financeiros carregados."""
        if self.file_data:
            try:
                resposta = self.cateline_lacet_gpt.get_financial_analysis_response(self.file_data, horas_trabalhadas=40)
                self.result_display.append(f"<p style='color: yellow;'>Análise Financeira:\n{resposta}</p>")
            except Exception as e:
                self.result_display.append(f"<p style='color: red;'>Erro ao realizar a análise financeira: {str(e)}</p>")
        ...
        try:
            ...
            if self.text_processor.is_greeting(prompt):
                ...
                if 'finança' in analysis['keywords'] or 'dinheiro' in analysis['keywords']:
                    resposta = self.handle_financial_prompt(analysis)
                else:
                    resposta = "Eu não consigo responder a isso no momento."
