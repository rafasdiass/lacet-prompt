...
class DataPersistenceService:
    ...
    def save_prompt_and_response(self, prompt_text: str, response: str, source: str = "local"):
        ...
        try:
            ...
            raise RuntimeError(f"Erro ao salvar prompt e resposta no banco de dados: {str(e)}")
    ...
    def save_gpt4_response(self, prompt_id: int, gpt4_response: str):
        ...
        try:
            ...
            self.session.commit()
            print(f"Resposta do GPT-4 associada ao prompt {prompt_id} salva com sucesso.")
        except Exception as e:
            self.session.rollback()
            raise RuntimeError(f"Erro ao salvar resposta do GPT-4 no banco de dados: {str(e)}")
    ...
    def save_financial_analysis(self, categorias_custos: dict, total_custos: float, receita_projetada: float):
        ...
        try:
            ...
            self.session.commit()
            print(f"Análise financeira salva com sucesso para o usuário {self.usuario.nome}.")
        except Exception as e:
            self.session.rollback()
            raise RuntimeError(f"Erro ao salvar análise financeira no banco de dados: {str(e)}")
    ...
    def get_user_financial_data(self):
        ...
        """
        return self.session.query(Recebimento).filter_by(usuario_id=self.usuario.id).all()
